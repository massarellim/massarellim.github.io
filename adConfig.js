...
pbjs.cmd.push(function () {
    ...
    // Define the 15 ad unit codes requested by Prebid
    // We use 9 desktop slots that are on the page (leaving slots 10 and 11 to be non-Prebid)
    // plus 6 phantom slots not on the page (4 mobile + 2 generic phantom)
    let adUnitCodes = [];
    for(let i=1; i<=9; i++) adUnitCodes.push("/6353/test_desktop_" + i);
    for(let i=1; i<=4; i++) adUnitCodes.push("/6353/test_mobile_" + i);
    adUnitCodes.push("/6353/test_phantom_1", "/6353/test_phantom_2");
    ...
    // Cites: Using the function-based 'when' match rule shown in the user's documentation.
    let intercepts = [];
    
    // Criteo always has a rule for the first slot to enforce its large timeout
    intercepts.push({
